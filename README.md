...
- 控制面启动不会自动注册任何仿真节点；节点必须由手动启动的 CloudSimPlus、`sim-backend` 或真实 Agent 注册/心跳上报。
...
├─ start_tianjun.bat               # Windows 启动控制面并打开 Dashboard
├─ restart_tianjun.bat             # Windows 安全重启控制面
├─ configs/
...
控制面启动后节点集合为空。需要仿真节点时，请在第二个终端手动运行 CloudSim Plus DCI 实验；也可以使用自己准备的库存文件手动启动通用 `sim-backend`：
...
Windows 下也可在配置好 API key 后双击 `start_tianjun.bat`。该脚本会先执行 LLM 连接检查，再启动控制面并打开 Dashboard。它不会自动启动任何仿真节点。`restart_tianjun.bat` 仅重启控制面，不会接管手动运行的仿真进程。
...
- 控制面刚启动时 `/report` 中节点集合为空；手动启动 CloudSimPlus、`sim-backend` 或真实 Agent 后才会出现节点。
...
- `--inventory path\to\your_sim_inventory.json`：仅校验一个可选的用户仿真库存配置，不会自动注册节点。
...
用户自备的 `sim-backend` 库存文件可描述：

- 分布在不同城市或接入区域的 CPU/GPU 模拟节点。
- 节点容量、标签、加速卡、存储、网络、合规属性、价格和可靠性。
- 区域之间的时延、带宽、丢包和路径可靠性画像。

项目不再随附或自动加载固定实例库存。需要通用模拟节点时，使用 `--inventory path\to\your_sim_inventory.json` 手动启动 `sim-backend`。
...
| `python main.py sim-backend` | 手动启动配置驱动的模拟节点运行时 |
...
- CloudSimPlus 与手动配置的模拟节点代表仿真资源，不能等同于真实物理资源库存。
...
- 增加 Docker Compose 或部署脚本，使控制面和 Dashboard 可一键运行，并保持仿真节点按需手动接入。
